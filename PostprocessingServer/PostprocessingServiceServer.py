...
async def download_inference_outputfiles(request: DagRequest):
    ...
    dag_root_folder_name = f"{dag_id}_{execution_id}"
    ...
    dag_root_folder = Path.home() / "Desktop" / dag_root_folder_name
    root_folder_path = Path(dag_root_folder)
    postprocessing_folder_path = root_folder_path / "NCU-RSS-Predict-Postprocessing"
    temp_download_folder = postprocessing_folder_path / "temp_postprocessing_download"
...
async def upload_log_to_s3(request: DagRequest):
    ...
    try:
        # Log 檔案路徑
        log_file_path = Path.home() / "Desktop" / dag_root_folder_name / "LOGS" / f"{dag_id}_{execution_id}.txt"
